...
Original file is located at
    https://colab.research.google.com/drive/1bbXSEu-QjD8EIJvPTOOB11EoGMn1G209
"""
...
collection = client.get_or_create_collection("apu32_functions")

# Load JSON data
import json
with open('/content/vectordb_data.json', 'r') as f:
    data = json.load(f)

# Iterate over each function, generate embeddings, and store in Chroma DB
for i, function in enumerate(data):  # Use enumerate to get unique IDs for each function
    # Convert the 'description' field to embeddings
    embedding = model.encode(function["description"]).tolist()  # Convert embedding to list

    # Convert the list fields (like 'parameters') to strings
    parameters_str = ', '.join(function["parameters"])  # Convert list to a comma-separated string

    # Store in Chroma DB with relevant metadata
    collection.add(
...
# Define the search query
#search_query = "Measure current on APU-32 pins"
search_query = "list the parameters of apu32set() function"
# Encode the search query into an embedding
query_embedding = model.encode(search_query).tolist()  # Convert to list
